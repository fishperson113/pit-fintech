"""ADR-010 -- the event-based write path: publish post-score updates to a Redis Stream.

`/score` is a **publisher**, never a store mutator. It publishes a score event to a Redis Stream
(`pit:{feature_service_version}:events`) for the dedicated `pit-online-worker` container to apply
under the optimistic lock, update the online store and append the Event History. The worker also
publishes a short-lived diagnostic result, but the scoring request does not wait for it.
...
#: Result key the worker writes the pre-decision feature vector to for diagnostics.
SCORE_RESULT_KEY_TEMPLATE: Final = "pit:{feature_service_version}:result:{request_id}"
#: Result TTL (seconds): short-lived diagnostic data, not an online state store.
