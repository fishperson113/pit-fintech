...
| Model | ALS | XGBoost + Ray Tune | TBD sau PaySim EDA; LightGBM là candidate |
...
| Local footprint | Nhiều distributed services | 37 compose service definitions | 3 core services; optional MinIO, observability tách VPS |
...
4. **Component lifecycle qua Makefile/Compose profiles**
   Có `make up-core`, `make status`, `make logs`, `make down` và profile optional cho MinIO.
   Observability runtime nằm trên VPS/ops boundary riêng, không thuộc core application Compose.
...
- OpenTelemetry instrumentation với remote VPS collector chỉ sau khi core parity ổn.
...
- Spark/Ray Train/Tune/Serve: dữ liệu, training và serving reference fit CPU single-node; cluster local không chứng minh cloud scalability.
...
Delta/DuckDB/model đã chọn chạy như batch processes, không phải long-running services. MinIO là
optional Compose profile; OTel Collector/Prometheus/Grafana self-host trên VPS nếu Sprint 3 còn
buffer và không trở thành release dependency.

Streaming được mô phỏng bằng một logical Replay Driver với ordered in-memory queue/iterator. Không
thêm Redpanda/Kafka/RabbitMQ/Redis Streams hoặc Debezium trong sáu tuần MVP.
