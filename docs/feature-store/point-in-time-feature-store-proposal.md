...
- IEEE-CIS là workload fintech phù hợp nhưng không phải dataset nguyên bản của paper. Đề tài là **adapt-and-evaluate**, không claim tái lập toàn bộ FeathrPO hay con số speedup 3×.
...
- Stack mặc định: Delta Lake (`delta-rs`) trên Parquet làm lakehouse table format, DuckDB cho offline computation, Feast cho feature registry và retrieval contract, Redis/Upstash cho online store, LightGBM + MLflow cho training/evaluation, FastAPI cho scoring, Prometheus/Grafana cho quan sát, Docker Compose + Makefile + GitHub Actions cho reproducibility.
...
```mermaid
flowchart LR
    K["Kaggle IEEE-CIS archive"] --> I["Ingestion + schema checks"]
...
Nếu Modal hoặc Upstash không đăng ký được, `docker compose up` phải chạy toàn bộ API + Redis + MLflow + monitoring local. Cloud là bằng chứng bổ sung cho portability, không phải single point of failure của đồ án.
...
| Feature store | Feast | Feature registry, historical retrieval, materialization, online retrieval |
...
| Model | LightGBM | Mạnh với tabular, train CPU nhanh, model nhỏ |
...
| Monitoring | Prometheus + Grafana + structured JSON logs | Freshness, parity, latency, error, promotion/rollback và version audit |
...
`compose.yaml` dùng profile để core chỉ khởi động Redis, MLflow và FastAPI. Prometheus/Grafana và MinIO là optional profiles. Batch compute bằng DuckDB/delta-rs/LightGBM chạy theo command, không biến thành container thường trú.
...
- LightGBM baseline có MLflow tracking.
...
- Cross-runtime prediction parity và A/B benchmark Python LightGBM/FastAPI với TypeScript ONNX/Fastify trên cùng hardware/workload.
- Streaming replay với Redis Streams hoặc Redpanda.
...
| Scope creep MLOps | Thêm Kafka/K8s/model registry trước parity | Đóng băng nice-to-have tới khi Sprint 2 gate pass |
...
6. LightGBM model + MLflow experiment artifacts.
